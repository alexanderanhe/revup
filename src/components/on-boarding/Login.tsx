...
export default function Login({ buttonClass, buttonText, handleNext }: LoginProps) {
  ...
  const AuthButtons = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center max-w-sm w-full gap-2">
        <p>Loguearse con:</p>
    ...
    <Fragment>
      ...
      <footer className="grid grid-cols-1 place-items-center gap-2 pb-10">
        { status === "authenticated"
          ? <button type="button" onClick={handleNext} className="btn btn-success w-full">Continuar</button>
          : (
          <Fragment>
            <button type="button" onClick={handleNext} className={ buttonClass }>{ buttonText }</button>
            <p className="text-center">
              {'Al continuar acepta los '}
              <Link href="/terms" className="text-center text-blue-500">Terminos y condiciones</Link>
              {' de la aplicación.'}
            </p>
          </Fragment>
        )}
